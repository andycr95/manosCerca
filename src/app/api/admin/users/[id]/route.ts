...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (session.role !== "SUPERADMIN" && ([Role.SUPERADMIN, Role.ADMIN].includes(current.role) || [Role.SUPERADMIN, Role.ADMIN].includes(parsed.data.role))) return NextResponse.json({ error: "No tienes permiso para modificar cuentas administrativas." }, { status: 403 });
  try {
