...
type TrackingResult = { code: string; statusLabel: string; message: string; requestedAt: string; scheduledAt: string | null; completedAt: string | null; updatedAt: string };
...
export default function PublicRequestPage() {
  ...
  const checkTracking = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setTrackingLoading(true); setTrackingError(""); setTrackingResult(null);
    try {
      const response = await fetch(`/api/public-requests?code=${encodeURIComponent(trackingInput.trim().toUpperCase())}`, { cache: "no-store" });
      const result = await response.json() as { tracking?: TrackingResult; error?: string };
      if (!response.ok || !result.tracking) throw new Error(result.error || "No encontramos esa solicitud.");
      setTrackingResult(result.tracking);
    } catch (error) { setTrackingError(error instanceof Error ? error.message : "No pudimos consultar la solicitud."); } finally { setTrackingLoading(false); }
  };
  ...
  return <main className="public-page">
    ...
    <section className="tracking-access" aria-labelledby="tracking-title"><div><p className="public-eyebrow">Consulta de solicitud</p><h2 id="tracking-title">¿Ya tienes un número de radicado?</h2><p>Consulta el estado de tu solicitud sin crear una cuenta.</p></div><button type="button" className="tracking-toggle" onClick={() => { setTrackingOpen((current) => !current); setTrackingError(""); }}>{trackingOpen ? "Cerrar consulta" : "Consultar radicado"} <span>{trackingOpen ? "↑" : "→"}</span></button>{trackingOpen && <form className="tracking-form" onSubmit={checkTracking}><label>Número de radicado<input value={trackingInput} onChange={(event) => setTrackingInput(event.target.value.toUpperCase())} placeholder="Ej. AYU-2026-000001" autoCapitalize="characters" required /></label><button className="public-primary" disabled={trackingLoading}>{trackingLoading ? "Consultando…" : "Consultar estado"} <span>→</span></button>{trackingError && <p className="form-error" role="alert">{trackingError}</p>}{trackingResult && <div className="tracking-result"><span className="tracking-state">{trackingResult.statusLabel}</span><strong>{trackingResult.code}</strong><p>{trackingResult.message}</p><small>Recibida el {new Date(trackingResult.requestedAt).toLocaleDateString("es-CO", { dateStyle: "long" })}{trackingResult.scheduledAt ? ` · Gestión programada: ${new Date(trackingResult.scheduledAt).toLocaleDateString("es-CO", { dateStyle: "long" })}` : ""}</small></div>}</form>}</section>
    ...
    {!mode ? <section className="public-choice" aria-labelledby="choice-title"><div className="choice-heading"><p className="public-eyebrow">Paso 1 de 1</p><h2 id="choice-title">¿Para quién solicitas ayuda?</h2><p>Elige la opción que más se parezca a tu caso.</p></div><div className="choice-grid"><button className="choice-card" onClick={() => setMode("self")}><span className="choice-art self-art">♡</span><div><strong>Para mí o mi familia</strong><p>Necesito apoyo para mi hogar.</p></div><span className="choice-arrow">→</span></button><button className="choice-card" onClick={() => setMode("leader")}><span className="choice-art leader-art">♧</span><div><strong>Soy líder o lideresa</strong><p>Quiero solicitar apoyo para otra persona, familia o comunidad.</p></div><span className="choice-arrow">→</span></button></div></section> : <section className="public-form-wrap"><div className="form-top"><button className="back-button" onClick={() => step === 1 ? setMode(null) : setStep((current) => current - 1)}>← <span>Volver</span></button><div className="progress"><span className={step >= 1 ? "done" : ""} /><span className={step >= 2 ? "done" : ""} /><span className={step >= 3 ? "done" : ""} /></div><small>Paso {step} de 3</small></div>
      {step === 1 && <form className="public-form" onSubmit={continueForm}><p className="public-eyebrow">Datos y ubicación</p><h2>{mode === "leader" ? "Tus datos y el lugar de la ayuda" : "Cuéntanos de ti y de tu ubicación"}</h2><p className="form-intro">Así podremos contactarles y dirigir la ayuda al territorio correcto.</p><label className="form-label">{mode === "leader" ? "Tu nombre completo" : "Nombre de la persona que solicita"}<b> *</b><input value={applicantName} onChange={(event) => setApplicantName(event.target.value)} placeholder="Escribe tu nombre" required /></label><label className="form-label">Número de celular<b> *</b><input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Ej. 300 123 4567" type="tel" required /></label><SearchableSelect label="¿Cómo prefieres que te contactemos?" placeholder="Selecciona una opción" value={preferredContact} onChange={setPreferredContact} options={[{ value: "llamada", label: "Por llamada" }, { value: "whatsapp", label: "Por WhatsApp" }, { value: "mensaje", label: "Por mensaje de texto" }]} />{mode === "leader" && <label className="form-label">Nombre de la persona o familia que necesita ayuda<b> *</b><input value={beneficiaryName} onChange={(event) => setBeneficiaryName(event.target.value)} placeholder="Ej. Familia Riascos" required /></label>}<div className="form-section-title"><span>⌖</span><div><strong>Ubicación de la ayuda</strong><small>Selecciona departamento, municipio y tipo de zona.</small></div></div><SearchableSelect label="Departamento" required placeholder="Busca tu departamento" value={department} options={departmentOptions} onChange={(value) => { setDepartment(value); setMunicipality(""); setAreaType(""); setNeighborhood(""); setRuralDistrict(""); setVillage(""); setManualLocation(""); }} /><SearchableSelect label="Municipio" required placeholder={department ? "Busca tu municipio" : "Primero selecciona un departamento"} value={municipality} options={municipalitySelectOptions} disabled={!department} onChange={(value) => { setMunicipality(value); setAreaType(""); setNeighborhood(""); setRuralDistrict(""); setVillage(""); setManualLocation(""); }} emptyMessage="No encontramos un municipio con ese nombre." /><fieldset className="area-choice" disabled={!municipality}><legend>¿La ayuda se necesita en zona urbana o rural? <b>*</b></legend><label className={areaType === "urban" ? "selected" : ""}><input type="radio" name="area" value="urban" checked={areaType === "urban"} onChange={() => { setAreaType("urban"); setRuralDistrict(""); setVillage(""); setManualLocation(""); }} /> <span>⌂</span><strong>Urbana</strong><small>En la ciudad o cabecera municipal</small></label><label className={areaType === "rural" ? "selected" : ""}><input type="radio" name="area" value="rural" checked={areaType === "rural"} onChange={() => { setAreaType("rural"); setNeighborhood(""); setManualLocation(""); }} /> <span>⌇</span><strong>Rural</strong><small>En vereda, corregimiento o zona rural</small></label></fieldset>{isBuenaventura && areaType === "urban" && <SearchableSelect label="Barrio de Buenaventura" required placeholder="Busca tu barrio" value={neighborhood} options={neighborhoodOptions} onChange={setNeighborhood} description="Barrios organizados por localidad y comuna." emptyMessage="No encontramos ese barrio." />}{isBuenaventura && areaType === "rural" && <><SearchableSelect label="Corregimiento" required placeholder="Busca el corregimiento" value={ruralDistrict} options={ruralDistrictOptions} onChange={(value) => { setRuralDistrict(value); setVillage(""); }} /><SearchableSelect label="Vereda" required placeholder={ruralDistrict ? "Busca la vereda" : "Primero selecciona el corregimiento"} value={village} options={villageOptions} disabled={!ruralDistrict} onChange={setVillage} description="Veredas organizadas por corregimiento." emptyMessage="No encontramos esa vereda." /></>}{!isBuenaventura && areaType && <label className="form-label">{areaType === "urban" ? "Barrio o sector" : "Vereda, corregimiento o sector"}<b> *</b><input value={manualLocation} onChange={(event) => setManualLocation(event.target.value)} placeholder={areaType === "urban" ? "Ej. Centro" : "Ej. Vereda El Porvenir"} required /></label>}<label className="form-label">Dirección o referencia <small>Opcional</small><input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="Ej. Cerca a la cancha, casa azul" /></label><FormContinue text="Continuar" disabled={!locationIsComplete} /></form>}
      ...
      {step === 3 && <form className="public-form" onSubmit={submitRequest}><p className="public-eyebrow">Revisa tu solicitud</p><h2>Todo listo para enviar</h2><p className="form-intro">Verifica la información antes de enviarla. El equipo recibirá tu solicitud de inmediato.</p><div className="request-review"><div><span>Solicita</span><strong>{mode === "leader" ? beneficiaryName : applicantName}</strong></div><div><span>Contacto</span><strong>{phone}</strong></div><div><span>Ubicación</span><strong>{selectedDepartmentName} · {selectedMunicipalityName}</strong><small>{areaType === "urban" ? "Zona urbana" : "Zona rural"} · {selectedLocationName}{ruralDistrict ? ` · ${ruralDistrict}` : ""}</small></div><div><span>Ayuda solicitada</span><strong>{selectedNeeds.join(" · ")}</strong></div></div>{submissionError && <p className="form-error" role="alert">{submissionError}</p>}<div className="consent"><span>✓</span><p>La información será usada únicamente para gestionar esta solicitud de ayuda.</p></div><button className="public-primary" type="submit" disabled={submitting}>{submitting ? "Enviando…" : "Enviar solicitud"} <span>→</span></button></form>}</section>}
